...
def log_message(conv_id: str, role: str, content: str, intent_label: str = None):
    """Log a user or assistant message to the database and increment message count."""
    msg_id = str(uuid.uuid4())
    conn = get_db_connection()
    try:
        conn.execute(
            "INSERT INTO messages (msg_id, conv_id, role, content, intent_label, sent_at) VALUES (?, ?, ?, ?, ?, ?)",
            (msg_id, conv_id, role, content, intent_label, datetime.now().isoformat())
        )
        conn.execute(
